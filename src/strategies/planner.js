...
import { DeliveryStrategy } from "./delivery.js";
...
export class Planner {
    ...
    async getAction() {
        if (!this.canAct()) return { action: ACTIONS.NONE};
        
        // If parcel at current position, handle it reactively
        let reactive_action = await this.handleReactivePickup();
        if (reactive_action != ACTIONS.NONE) {
            return {action: reactive_action};
        }
    ...
    async handleReactivePickup() {
        const parcelAtCurrentPos = this.beliefs.getParcelAtCurrentPosition();
        
        if (!parcelAtCurrentPos) return ACTIONS.NONE;
    ...
    shouldAvoidHandoverPickup(parcel) {
        return this.handoverCoordinator.handoverTile &&
               parcel.x === this.handoverCoordinator.handoverTile.x &&
               parcel.y === this.handoverCoordinator.handoverTile.y &&
               this.handoverCoordinator.isRunner();
    }
    ...
    isRunner() {
        return this.handoverCoordinator.isRunner();
    }

    setHandoverTile(handoverTile) {
        this.handoverCoordinator.handoverTile = handoverTile;
    }
